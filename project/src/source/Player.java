...
public class Player extends Person{
	...
	private float startTimeForRadarBonus, startTimeForSpeedUpBonus, startTimeForStonedBonus, startTimeForJumpBonus;
	 ...
	 * @param dir
	 * @throws BozorgExceptionBase: directionIsBlocked, IncompleteOperation, hasStonedBonus
	 */
	public void move(int dir) throws BozorgExceptionBase{
		...
		if(hasJumpBonus > 0){
			nextCell = Cell.getNextCellFromDirNoBlockedWay(super.getCell(), dir);
		}
		...
		isMoving = true;
		startTimeForMoving = Judge.getStaticTime();
	...
	/**
	 * player.health -= x;
	 * player will die if x >= player.health
	 * @param power
	 */
	 ...
	 * 
	 * @param dir
	 * @throws BozorgExceptionBase :  hasStonedBonus, directionIsBlocked, IncompleteOperation
	 */
	public void attack(int dir) throws BozorgExceptionBase{
		...
		isAttacking = true;
		startTimeForAttacking = Judge.getStaticTime();
	}
	
	private void finishAttacking(){
		for (Person tmp : enemy){
			if (tmp.equals(this)) // if enemy is this person ( attacking curret cell )
				continue;
	...
	public void getGift() throws BozorgExceptionBase{
		...
		if (this.getCell().getType() == JudgeAbstract.HOSPITAL_CELL){
			if (this.getCell().getAllPlayers().size() != 1)
				throw new MoreThanOnePlayerWantToGetAGift();
			this.getCell().setType(JudgeAbstract.NONE_CELL);
			getHospitalBonus();
		}
		if (this.getCell().getType() == JudgeAbstract.FAN_CELL){
			if (this.getCell().getAllPlayers().size() != 1)
				throw new MoreThanOnePlayerWantToGetAGift();
			this.getCell().setType(JudgeAbstract.NONE_CELL);
			getFanBonus();
		}
		if (this.getCell().getType() == JudgeAbstract.JUMP_CELL){
			if (this.getCell().getAllPlayers().size() != 1)
				throw new MoreThanOnePlayerWantToGetAGift();
			this.getCell().setType(JudgeAbstract.NONE_CELL);
			startHavingJumpBonus();
		}
		if (this.getCell().getType() == JudgeAbstract.SPEEDUP_CELL){
			if (this.getCell().getAllPlayers().size() != 1)
				throw new MoreThanOnePlayerWantToGetAGift();
			this.getCell().setType(JudgeAbstract.NONE_CELL);
			startHavingSpeedUpBonus();
		}
		if(this.getCell().getType()==JudgeAbstract.STONE_CELL){
			if (this.getCell().getAllPlayers().size() != 1)
				throw new MoreThanOnePlayerWantToGetAGift();
			this.getCell().setType(JudgeAbstract.NONE_CELL);
	...
	public boolean isCellInVision(Cell cell){
		...
		if (Math.abs(cell.getRow() - this.getCell().getRow()) <= this.getVision()
				&&
				Math.abs(cell.getCol() - this.getCell().getCol()) <= this.getVision())
			return true;
...
//				fan.setAlive(false);
	...
	public HashMap<String, Integer> getInfo(){
		...
		ret.put(JudgeAbstract.IS_WINNER, !this.gameIsOver() ? JudgeAbstract.NOT_FINISHED : winner.getName());
	...
	public void continueCurrentOperation(){
		float time = Judge.getStaticTime();
...
//		if (this.getCell().getType() == JudgeAbstract.JJ_CELL ){//check if game has finished
//			ArrayList<Player> players = this.getCell().getAllPlayers();
//			if (players.size() == 1){
//				Player.setWinner(this);
//			}
//		}
	}
